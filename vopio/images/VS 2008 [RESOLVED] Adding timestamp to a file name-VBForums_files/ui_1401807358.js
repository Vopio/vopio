...
var duration=parseInt($iTXT.glob.dbParams.get("hk.autopeek.duration",3000));var repeat=$iTXT.glob.params.getBool('hk.autopeek.repeat',false);function flipHook(hk,duration){if(hk.ad.isDynamicAdhesion()){hk.ad.params.set('hk.autopeek.on',true,$iTXT.cnst.WEIGHTING_DEFAULT_CAMPAIGN);}else if(!hk.ad.params.getBool('hk.autopeek.on',true)){return;}
if(!$iTXT.ui.currentAutoPeek){if(!hk.flipped||repeat){hk.flipped=true;$iTXT.ui.AutoPeek.display(hk);var afterTime=$itxtUtil.ts()-masterTs;$iTXT.fire("$iTXT:data:log:monitor",{advert:hk.ad,mt:136,mv:hk.initPos.top+','+viewportHeight+','+pageHeight,mv2:afterTime});}}}
...
if($iTXT.core.Browser.isSmartphoneOrTablet(false,false)){var zoom=$iTXT.glob.params.get("hk.autopeek.zoom",0);var throttle=+($iTXT.glob.params.get("hk.autopeek.throttle",500));var flip=VM._.throttle(function(){if($iTXT.core.Browser.is("Android",2.3,2)){monitor($HookMngr.visibleHooks(),0,flipHook);}else{var bounds=$iTXT.ui.AutoPeek.calculate_bounds();monitor($HookMngr.visibleHooks(bounds,zoom),0,flipHook);}},throttle);$iTXT.core.$(window).itxtSubscribe('scroll',flip);VM._.each($iTXT.ui.HookManager.hooks,function(hook){hook.ad.params.set("cts",0,99999);if(hook.ad.params.getBool('hk.autopeek.on',true)){hook.ad.params.set("hk.click.mode",0,$iTXT.cnst.WEIGHTING_DEFAULT_CAMPAIGN);}else{if(hook.ad.params.get('hk.click.mode')==0||!hook.ad.params.get('hk.click.mode')){hook.ad.params.set("hk.click.mode",1,$iTXT.cnst.WEIGHTING_DEFAULT_CAMPAIGN);}}});}});$iTXT.ui.HookManager={id:0,hookedCounter:0,textNodes:[],hooks:[],startIndex:0,unHookedHooks:[],initSkip:0,maxKWNode:5,maxKWPara:5,keywordPadding:0,keywordInterval:0,placePerParagraph:0,paragraphCounts:{},specialChars:{},nodeID:1,add:function(hook)
{if(hook)
...
{if($iTXT.glob.params)
{var gps=$iTXT.glob.params;var wdb=$iTXT.cnst.WEIGHTING_DEFAULT_DATABASE;gps.set("hk.class","itxthook",wdb);gps.set("hk.class.active","itxthookactive",wdb);gps.set("hk.icon","",wdb);gps.set("hk.icon.active","",wdb);gps.set("hk.icon.path","http://images.intellitxt.com/ast/adTypes/",wdb);gps.set("fg","#006400",wdb);gps.set("bg","transparent",wdb);gps.set("hk.fg.col","${fg}",wdb);gps.set("hk.fg.h.col","#006400",wdb);gps.set("hk.bg.col","transparent",wdb);gps.set("hk.bg.h.col","${bg}",wdb);gps.set("hk.def.style","text-decoration: underline; border-bottom: 1px solid ${hk.fg.col}; border-top: none; color: ${hk.fg.col}; background-color: ${hk.bg.col}",wdb);gps.set("hk.def.h.style","text-decoration: underline; border-bottom: 0.2em solid ${hk.fg.h.col}; border-top: none; color: ${hk.fg.h.col}; background-color: ${hk.bg.h.col}",wdb);gps.set("hk.style","${hk.def.style}",wdb);gps.set("hk.h.style","${hk.def.h.style}",wdb);}},_onNoHook:function(){if(this.hooks.length){return;}
...
this.visited=false;if(this.ad.params.getBool('js.temp.mobt',false)){this._scheduleClose();}
...
$iTXT.ui.TooltipPosition={AR:"AR",AC:"AC",AL:"AL",CL:"CL",CR:"CR",BR:"BR",BC:"BC",BL:"BL"};$iTXT.ui.TooltipPlacer={STEP_LENGTH_PX:1,TAIL_WIDTH:30,weight:{AC:90,AL:85,AR:80,BC:70,BL:65,BR:60},_getHookPosition:function(hook){if(hook.hasMobt()){return $iTXT.core.Util.getPosition(hook.mobtCover);}
return hook.getPosition();},_getHookSize:function(hook){var el=hook.rootElement;if(hook.hasMobt()){el=hook.mobtCover;}
return{width:el.offsetWidth,height:el.offsetHeight};},_getHookCenterPosition:function(hook){var pos=this._getHookPosition(hook),size=this._getHookSize(hook);var yTopModifier=hook.hasMobt()?-2:0;return{top:{y:pos.top+yTopModifier,x:Math.round(pos.left+size.width/2)},bottom:{y:pos.top+size.height,x:Math.round(pos.left+size.width/2)}};},initHotZones:function(){var i,j,avoid,nodes;this.hotZones=[];avoid=['iframe','object','embed'];for(i=0;i<avoid.length;i++){nodes=document.getElementsByTagName(avoid[i]);for(j=nodes.length-1;j>=0;j--){var pos=$iTXT.core.Util.getPosition(nodes[j]);this.hotZones.push({top:pos.top,right:pos.left+nodes[j].offsetWidth,bottom:pos.top+nodes[j].offsetHeight,left:pos.left});}}
...
{this.ttWidth=opts.tt.width;this.ttHeight=opts.tt.height;initDebug();debug.clear();this.initScreenBounds();this.initHotZones();return this._getTooltipPlacement(opts.hk);},getTopCenterPlacement:function(hook,dimensions){var pos;if(!(hook instanceof $iTXT.ui.Hook)||!VM._.isObject(dimensions)||!VM._.isNumber(dimensions.width)||!VM._.isNumber(dimensions.height)){return null;}
